...
def make_bucket_sep_index(bucket, return_index=False):
...
# %%
def make_buckets(buckets, return_index=False):
    xys = Parallel(n_jobs=8)(delayed(make_bucket_sep_index)(buc, return_index) for buc in tqdm(buckets))
    idxs, Xs, ys = [], [], []
    for xy in xys:
        if return_index:
            X, y, idx = xy
